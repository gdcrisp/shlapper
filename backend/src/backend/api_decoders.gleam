import backend/shared_types.{
  type CreateProjectRequest, type CreateTaskRequest, type UpdateProjectRequest,
  type UpdateTaskRequest, CreateProjectRequest, CreateTaskRequest,
    ...
    Ok(#(name, description, deadline, status_str, color_str)) -> {
      ...
      case shared_types.project_status_from_string(status_str) {
        Error(_msg) ->
          Error([
            decode.DecodeError(
              expected: "valid project status",
              found: status_str,
              path: ["status"],
            ),
          ])
        Ok(status) ->
          case shared_types.project_color_from_string(color_str) {
            Error(_msg) ->
              Error([
                decode.DecodeError(
                  expected: "valid project color",
                  found: color_str,
                  path: ["color"],
                ),
              ])
    ...
    Ok(#(
      ...
      case shared_types.task_status_from_string(status_str) {
        Error(_msg) ->
          Error([
            decode.DecodeError(
              expected: "valid task status",
              found: status_str,
              path: ["status"],
            ),
          ])
        Ok(status) ->
          case shared_types.task_priority_from_string(priority_str) {
            Error(_msg) ->
              Error([
                decode.DecodeError(
                  expected: "valid task priority",
                  found: priority_str,
                  path: ["priority"],
                ),
              ])
